...
| `GET` | `/api/auth/session` | Current user, workspace, role, MFA state. |
| `POST` | `/api/auth/mfa/enroll` | Returns `otpauth://` URI + backup codes; requires a valid code to activate. |
| `POST` | `/api/auth/mfa/disable` | Requires current password. |
| `POST` | `/api/auth/password` | Changes password; revokes all other sessions. |
...
| `PATCH`/`DELETE` | `/api/entities/{id}` | Delete is refused (`409`) while accounts reference it. |
| `GET`/`PUT` | `/api/settings/cash-reserves` | Minimum cash thresholds per scope. |
...
| `GET` | `/api/link/items` | Linked items with status and last sync time. Never returns tokens. |
...
| `GET` | `/api/sync/status` | Per-item last sync, cursor age, and error state. |

## Accounts

| Method | Path | Notes |
| --- | --- | --- |
| `GET` | `/api/accounts` | Filters: `ledger`, `entityId`, `classification`, `type`, `includeClosed`. |
| `POST` | `/api/accounts` | Creates a **manual** account (property, vehicle, cash, private loan). |
...
| `DELETE` | `/api/accounts/{id}` | Manual accounts only. |
...
| `POST` | `/api/transactions/bulk` | Same fields across a set of ids. |
| `GET`/`POST` | `/api/rules` | Merchant rules (category, entity, ledger, cadence, funding account). |
| `POST` | `/api/rules/{id}/apply` | Backfills the rule across existing history. |
...
| `POST` | `/api/recurring/{id}/promote` | Creates a `Bill` from a detected series. |
| `GET`/`POST` | `/api/bills` | List / create. |
| `PATCH`/`DELETE` | `/api/bills/{id}` | Includes `autopay` toggle. |
| `GET` | `/api/bills/calendar` | Occurrences for a month, shaped for the calendar grid. |
| `POST` | `/api/bills/{id}/occurrences/{occurrenceId}/mark-paid` | Manual tracking only. Records actor and time. Does **not** create a `Payment`. |
| `POST` | `/api/bills/{id}/occurrences/{occurrenceId}/skip` | |
...
| `POST` | `/api/payments/{id}/cancel` | Allowed before `SUBMITTED`. |
...
| `POST` | `/api/properties/{id}/valuations` | Appends a valuation. `source: MANUAL` today; the automated-valuation path writes here later. |
| `GET` | `/api/investments` | Holdings grouped by account. |
| `GET` | `/api/reports/pnl` | Business P&L. Query: `entityId`, `period=month\|quarter\|year`, `from`, `to`, `groupBy=category`. |
| `GET` | `/api/reports/cash-flow` | Inflow/outflow series by ledger and period. |
| `GET` | `/api/reports/net-worth` | Trend from balance snapshots. |
| `GET` | `/api/insights` | Active insights, severity ordered. |
